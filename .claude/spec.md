...
## 2026-05-07 V2 配置同步补充

1. 右侧配置面板中的 WoS 参数、邮箱和模型设置，不应要求用户必须先点击“保存到 .env”才会影响当前对话。
2. 当用户修改了右侧配置后直接发送消息，前端应先自动同步这些配置，再请求 Agent 生成待确认动作或执行上传动作。
3. WoS 待确认动作中的“最多读取多少封邮件 / 最多处理多少页”必须来自用户当前界面可见的最新配置，而不是旧的已保存值。
4. 手动“保存到 .env”按钮仍然保留，用于显式长期保存配置；但当前会话中的实际执行参数应以用户眼前设置为准。
5. 邮箱配置中的 `EMAIL_PROVIDER=auto` 表示“按邮箱地址自动识别运营商”。后端在读取邮箱配置时必须先把 `auto` 解析为具体 provider，再查找 IMAP 配置，不能直接把 `auto` 当成真实运营商。

## 2026-05-07 V2 体验修订（聊天工作台）

1. React/FastAPI 主界面继续沿用 chat-first，但布局调整为：左侧为对话导航和高频能力提示，中间为聊天主工作区，右侧为用户配置；左侧不再暴露内部工具细节。
2. PDF 上传入口固定并入底部聊天输入区，不再放在页面顶部。用户滚动消息时，上传能力仍然可见。
3. WoS 配置必须使用用户能看懂的文案，不允许出现无说明的开关或数字输入框。至少明确：
   - 是否使用浏览器补全 WoS 完整结果页
   - 最多读取多少封 WoS 邮件
   - 最多处理多少页 WoS 结果
4. 当用户请求“筛选 WoS 邮件”时，待确认动作必须明确写出本次计划的范围，例如最多几封邮件、是否使用浏览器、最多几页。
5. 点击确认执行后，待确认卡片应立即消失，转为任务执行态，并持续流式显示工作日志，避免界面看起来像卡住。
6. 后台任务必须支持用户中断。界面提供“中断”按钮；后端 job manager 维护取消状态，并在下一次可中断点停止任务，返回“任务已取消”。
7. 聊天输入区不再出现“入库”这种需要用户预判的生硬选项。长期记忆写入应通过待确认动作或后续显式反馈完成，而不是默认绑在每次发送消息上。
8. WoS 推荐结果卡片必须突出显示：标题、DOI、作者、期刊/会议、摘要、推荐理由、WoS/Publisher 链接，以及“建议手动下载 PDF 后上传深读”。
9. WoS 推荐结果中的 DOI 不能只作为“尽力而为”的字段。对最终展示给用户的推荐论文，系统应在排序完成后执行一轮定向 DOI 补全：优先保留 WoS 已有 DOI；缺失时在浏览器模式下可按需进入该篇 Full Record 只抓 DOI；仍缺失时再调用公开元数据源补全。
10. 推荐结果需要向用户暴露 DOI 状态与来源，至少区分：`wos`、`full_record`、`crossref/openalex/semantic_scholar`、`missing`，避免用户无法判断“未获取到 DOI”到底是论文本身没有还是系统漏抓。
11. WoS 推荐卡片中的外链文案必须明确目标：优先显示“打开 WoS 记录”并指向该论文 Full Record；只有没有记录链接但有结果页链接时，才显示“打开 WoS 结果页”。不得用“打开记录”这类含糊文案。
12. 推荐结果 DOI 补全应优先复用本地历史论文库中同标题论文的 DOI/作者/期刊信息，再进入浏览器或公开元数据源，避免同一篇论文在不同轮次中一会儿有 DOI、一会儿没有 DOI。
13. 长期记忆需要有自然的 UI 入口：用户可在推荐卡片上标记“相关/不相关/记住论文”，分别写入 `interest_memory` 或 `paper_corpus`；写入后右侧记忆计数应刷新。默认批量筛选仍不自动写入长期记忆。
...
# 2026-05-06 重构：对话式学术 Agent 应用

项目定位从“文献追踪助手/自动下载流水线”调整为“本地对话式学术助手”。用户通过聊天框与 Agent 交流，上传 PDF、请求论文解读、请求 WoS Alert 候选筛选、检索历史论文与研究偏好、生成阶段性报告。自动出版商下载、SPIS 下载、文献求助和邮箱等待 PDF 不再属于默认主流程。

新架构遵循以下原则：
1. Agent 是主体：对话层负责理解意图、提出行动计划、等待关键动作确认、调用工具、总结结果。
2. 旧能力沉淀为工具：PDF 解读、WoS 候选提取、兴趣筛选、报告生成、历史检索作为可注册工具存在。
3. PDF 全文由用户主动上传或放入本地合法来源，不再自动绕过网页、人机验证、机构认证或付费墙。
4. 关键动作必须先确认：长期记忆写入、批量邮箱读取、批量筛选、报告生成等动作需先说明计划并由用户确认。
5. 记忆分两层：`paper_corpus` 保存论文知识，`interest_memory` 保存用户研究兴趣、方法偏好、排除方向和反馈摘要。
6. 向量数据库第一版使用 Chroma；若本地未安装 Chroma，应用允许退回 JSON/内存式降级存储以保证界面可打开，但正式能力以 Chroma 为目标。
7. 长期记忆不完整保存聊天原文，只保存从对话中提炼出的研究偏好和用户明确反馈。
8. 学习机制采用混合学习：显式反馈权重最高，上传/深读等隐式行为作为低权重线索。
9. Streamlit 第一屏改为聊天界面；旧“一键周报/自动下载”式入口不再作为默认体验。

第一版模块边界：
- `paper_analyzer/agent/`：Agent runtime、工具注册、记忆管理、会话状态、待确认动作。
- `data/memory/chroma/`：Chroma 本地向量库目录。
- `data/conversations/`：本地会话运行产物。
- `data/library/`：用户上传并允许保存的 PDF 与论文资料。

默认工具：
- `analyze_pdf_tool`：分析用户上传 PDF。
- `screen_wos_alert_tool`：读取 WoS Alert 候选并给出推荐，不下载全文。
- `search_memory_tool`：检索论文库和兴趣记忆。
- `update_memory_tool`：根据用户反馈更新兴趣记忆。
- `generate_report_tool`：将对话或分析结果整理为 Markdown 报告。

# 2026-05-07 V2：React/FastAPI 聊天式前端

第二版默认前端从 Streamlit 切换为 React + FastAPI。Streamlit 只保留为 legacy/debug 入口，不再作为主体验。

V2 目标：
1. 主界面为对话式学术助手，不展示内部工具列表。
2. 左侧展示用户配置与记忆状态：邮箱、授权码、LLM provider/API key/base URL/model、研究主题、WoS 浏览器模式、最大邮件数和最大页数。
3. 用户保存配置时写入本地 `.env`，只更新允许字段，不提交真实 `.env`。
4. 对话输入框支持文字和 PDF 上传；PDF 上传不再放在页面顶部。
5. 待确认动作点击确认后立即消失，转为后台任务卡片，并通过 SSE/日志流持续输出工作进度。
6. WoS 筛选默认尝试“邮箱 + WoS 完整页”，复用 `fetch_papers(progress_callback=...)` 的完整抓取、摘要展开、DOI/元数据补全能力。
7. WoS 推荐输出必须包含标题、DOI、作者、期刊/会议、相似度/推荐分、完整摘要、推荐理由、WoS/Publisher 链接、复制 DOI 和手动下载 PDF 后上传深读的建议。
8. 后端新增 `paper_analyzer/server/`，提供 config/chat/upload/jobs/SSE API。
9. 前端新增 `frontend/`，使用 React + Vite + TypeScript 实现聊天布局、配置侧栏、待确认卡片、任务日志和推荐卡片。
10. 自动下载 PDF 仍不进入默认主流程，`FULL_TEXT_SOURCE=manual` 保持默认。
