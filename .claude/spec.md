...
4. 最终输出一篇面向用户阅读的文献周报，而不是仅展示中间论文列表。
5. 周报可在前端展示，并可选择推送到外部客户端。第一优先级为飞书自定义机器人 webhook；第二优先级为企业微信群机器人 webhook。个人微信直推暂不作为近期目标。
...
3. 点击“生成周报”后串联 `fetch-papers -> analyze_papers -> weekly report writer -> notifier`。
4. 周报输出包括：
   ...
   - 本周趋势/主题归纳
   - 对用户研究方向的启发与建议
5. 推送渠道优先实现 webhook 类低复杂度方案：
   - 飞书自定义机器人：首选，支持 webhook 单向推送，适合 Markdown/卡片化周报摘要。
   - 企业微信群机器人：次选，支持 webhook 单向推送，适合微信生态用户。
   - 个人微信/微信公众号直推：暂缓，原因是授权、审核、消息触达规则和非官方方案稳定性风险较高。
