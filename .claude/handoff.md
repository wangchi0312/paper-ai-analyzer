...
- Playwright WoS 浏览器模式默认使用 `data/browser_profiles/wos` 持久 profile，目录已加入 `.gitignore`；浏览器错误写入审计前会脱敏 URL query，避免保存邮箱、sid、loginId 等参数。
...
- 抓取审计针对性测试：`pytest -q tests/test_fetch_papers.py tests/test_email_reader.py tests/test_wos_browser.py -p no:cacheprovider`
  - 预期：当前为 `15 passed`
- 浏览器脱敏针对性测试：`pytest -q tests/test_fetch_papers.py tests/test_wos_browser.py -p no:cacheprovider`
  - 预期：当前为 `13 passed`
- Citation Alert 筛选与 Clarivate 登录针对性测试：`pytest -q tests/test_email_reader.py tests/test_fetch_papers.py tests/test_wos_browser.py -p no:cacheprovider`
  - 预期：当前为 `19 passed`
