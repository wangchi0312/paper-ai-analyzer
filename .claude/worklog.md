...
- 准备把前端配置状态上移到 `App`，并在发送消息前自动同步未保存的配置，避免“界面看到 2、实际执行还是 20”的错位。
- 继续排查后发现当前运行中的 `paper_analyzer/agent/runtime.py` 还存在更深一层问题：中文意图关键词发生编码损坏，导致“帮我筛选 WoS 邮件”没有进入 WoS 分支，而是退回普通聊天回复。

### 为什么
- 当前实现只有点击“保存到 .env”才会更新后端配置，而用户的自然预期是“我眼前刚改的设置，下一次执行就该生效”。
- 这个问题会直接破坏 WoS 筛选的可信度，因为待确认动作和真实执行范围都变得不可预测。
- 如果意图识别关键词本身损坏，即使配置同步修好了，WoS 筛选也根本不会真正触发。
